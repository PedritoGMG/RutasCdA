...
if (isset($userID) && isset($actividadID)) {
    $stmt = $conexion->prepare("UPDATE usuarioactividad SET escaneada = 1 WHERE userID = ? AND actividadID = ?");
    $stmt->bind_param("ss", $userID, $actividadID);
    $stmt->execute();

    if ($stmt->affected_rows > 0) {
        echo json_encode(["response" => true, "message" => "Escaneada actualizada correctamente"]);
    } else {
